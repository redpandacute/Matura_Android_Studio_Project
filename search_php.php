...
//conn Details
 ...
 //$user_school = $_POST["user_school"];
  ...
  $statement_string = "SELECT user_archive.*, user_subjects.* FROM user_archive INNER JOIN user_subjects ON user_archive.user_id = user_subjects.user_id WHERE (user_archive.user_name LIKE ? OR user_archive.user_username LIKE ? OR user_archive.user_firstname LIKE ?) AND user_archive.user_id != ?"; //TODO: Add school with "school = ? AND ..."
  ...
  mysqli_stmt_bind_result($statement, $id, $username, $name, $firstname, $school, $yearofbirth, $email, $password, $description, $id, $german, $spanish, $english, $french, $biology, $chemistry, $music, $maths, $physics);
	...
	if (mysqli_stmt_num_rows($statement) > 0) {
		...
		while($field = mysqli_stmt_fetch($statement)) {
			...
			$innerarray['user_yearofbirth'] = $yearofbirth;
