...
public class searchresults_activity extends AppCompatActivity {
    ...
    private void getProfilePictures(int start, int amount) {
        ...
        try {
            ...
            smallprofilepictures_request request = new smallprofilepictures_request(ids, new onPictureResponseListener());
            RequestQueue queue = Volley.newRequestQueue(searchresults_activity.this);
            ...
            for(loop = 0; loop < childCount; loop++) {
                ...
                if(holder.getInfo().getTempProfilePicturePath() == null && !isLoading) {
                    if(totalItemCount - firstVisibleItemPosition - 1 >= heapsize) {
                        ...
                        int amount = totalItemCount - firstVisibleItemPosition - 1;
                        int start = firstVisibleItemPosition + loop;
                        getProfilePictures(start, amount);
                        break;
                    }
